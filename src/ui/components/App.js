...
function render() {
  ...
  const tabs = renderTabs({
    ...
    onTabChange: (tabName) => {
      ...
      if (tabName === "reply") {
        ...
        setInputPlaceholder("Type your idea to get 5 polished posts...");
        // No longer auto-generating content when switching to Compose tab
      }
    },
  });
  ...
  // Then tab-specific content
    ...
    content = renderComposeTab({
      ...
      onUseTweet: (text) => useText(text), // Function to use the selected post
      isLoading: state.loading, // Use the global loading state
    });
  } 
...
function handleInputSend(text) {
  ...
  if (currentActiveTab === "reply") {
    ...
    // Generate polished posts based on user input
    if (text && text.trim()) {
      console.log("[App.js] Generating polished posts from user input:", text);
      generatePolishedPosts(text); // Pass user input to generate polished posts
    } else {
      // Inform user that input is required
      setError("Please enter your post idea before generating polished versions");
    }
    // Clear the input field after sending
    setCurrentInputText("");
  }
  // Schedule tab handling has been removed as it's not currently needed
}
...
async function handleGeneratePosts() {
  ...
  console.log(
    "[App.js] currentGlobalState.currentInput:",
    currentGlobalState.currentInput
  );
  console.log(
    "[App.js] Full currentGlobalState.settings from getState():",
    JSON.stringify(currentGlobalState.settings)
  );
  console.log(
    "[App.js] Checking API key from currentGlobalState.settings.apiKey:",
    currentGlobalState.settings
      ? currentGlobalState.settings.apiKey
      : "currentGlobalState.settings is undefined"
  );

  if (!currentGlobalState.settings || !currentGlobalState.settings.apiKey) {
    alert(
      "API key is not set. Please set it in the extension settings before generating posts."
    );
    console.warn(
      "[App.js] handleGeneratePosts: API key check failed. Aborting. currentGlobalState.settings was:",
      JSON.stringify(currentGlobalState.settings)
    );
    return;
  }
  ...
  try {
    const posts = await generateScheduledPosts(
      currentGlobalState.settings.apiKey,
      summary,
      currentGlobalState.settings
    );
...
async function handleSchedulePosts(postsToSchedule, scheduleDetail) {
  // scheduleDetail can be hours_from_now or a datetime-local string
  console.log("[App.js] handleSchedulePosts called with:", {
    postsToSchedule,
    scheduleDetail,
  });
  setError(null);
  ...
  if (postToSchedule.scheduledTime === "custom") {
    if (scheduleDetail && typeof scheduleDetail === "string") {
      // scheduleDetail is the datetime-local string
      const parsedCustomTime = new Date(scheduleDetail).getTime();
      if (isNaN(parsedCustomTime) || parsedCustomTime <= nowMs) {
        setError(
          "Invalid custom time provided or time is not in the future. Please enter a future date/time."
        );
        return;
      }
      whenToScheduleMs = parsedCustomTime;
      console.log(
        `[App.js] Custom schedule time raw string: ${scheduleDetail}, Parsed to ms: ${whenToScheduleMs}, Date: ${new Date(
          whenToScheduleMs
        )}`
      );
    } else {
  ...
  } else if (scheduleDetail && !isNaN(parseFloat(scheduleDetail))) {
    // scheduleDetail is hours from now ('1' or '2')
    ...
    console.log(
      `[App.js] Scheduling ${hoursFromNow} hours from now: ${new Date(whenToScheduleMs)}`
    );
  } else {
  ...
  try {
    ...
    console.log(
      `[App.js] Alarm created: ${alarmName} for time: ${new Date(whenToScheduleMs)}`
    );
    ...
    setMessage(
      `Post scheduled successfully for ${new Date(whenToScheduleMs).toLocaleString()}!`
    );
...
function generatePolishedPosts(userInput) {
  console.log("[App.js] Generating polished posts from user input");
  const state = getState();

  // Nothing to generate if no input
  if (!userInput || !userInput.trim()) {
    setError("Please enter your post idea first");
    return;
  }

  // Set loading state and clear any error
  setError(null);
  ...
  // Send message to background script to generate polished posts
  chrome.runtime.sendMessage(
    {
    ...
    (response) => {
      ...
      if (chrome.runtime.lastError) {
        console.error("[App.js] Error generating posts:", chrome.runtime.lastError);
        setError(`Error generating posts: ${chrome.runtime.lastError.message}`);
        return;
      }

      if (response && response.error) {
        console.error("[App.js] Error from posts generation:", response.error);
        setError(response.error);
        return;
      }

      if (!response || !response.ideas || !response.ideas.length) {
        setError("No post ideas were generated. Please try again.");
        return;
      }

      // Update state with generated post ideas
      updateState({
        tweetIdeas: response.ideas || []
      });

      console.log("[App.js] Generated polished posts:", response);
    }
  );
}
  ...
  if (state.isRecording) {
    ...
    try {
      ...
      permissionStatus.onchange = () => {
        console.log(
          "[App.js] Microphone permission status changed to:",
      ...
      if (permissionStatus.state === "denied") {
        alert(
          'Microphone access is permanently denied. Please go to your browser\'s site settings for this extension, change the microphone permission to "Allow" or "Ask", and then reload the extension.'
        );
        ...
        reader.onloadend = () => {
          ...
          chrome.runtime.sendMessage(
            ...
            (response) => {
              if (chrome.runtime.lastError) {
                console.error(
                  "[App.js] Error sending audio to background:",
                  chrome.runtime.lastError.message
                );
                setCurrentInputText("");
                setInputPlaceholder(
                  `Error: ${chrome.runtime.lastError.message.substring(0, 100)}`
                );
                setLoadingState(false);
                ...
                chrome.runtime.sendMessage(
                  ...
                  (polishResponse) => {
                    ...
                    if (chrome.runtime.lastError) {
                      console.error(
                        "[App.js] Error sending transcript for polishing:",
                        chrome.runtime.lastError.message
                      );
                      setInputPlaceholder(
                        `Polish error: ${chrome.runtime.lastError.message.substring(
                          0,
                          100
                        )}`
                      );
