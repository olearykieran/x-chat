...
/**
 * Generate 5 polished variations of a user's post idea or create 5 posts based on user's request
 * @param {string} apiKey - OpenAI API key
 * @param {string} userIdea - The user's raw post idea or content request
 * @param {object} userProfile - User's profile data (bio, writing samples)
 * @param {string} tone - Desired tone for the posts
 * @returns {Promise<Array<string>>} - Array of 5 post variations
 */
export async function generatePolishedPosts(apiKey, userIdea, userProfile) {
  ...
  // Check if the user's input appears to be a request for content creation rather than polishing
  const contentCreationPatterns = [
    /help me (create|make|write|craft|generate|come up with)/i,
    /can you (help|assist) me (promote|market|advertise)/i,
    /help me promote/i,
    /can you help me/i,
    /would you help me/i,
    /need (posts|content|tweets|ideas) (for|about)/i,
    /looking for (posts|content|tweets|ideas)/i,
    /generate (some|a few|5|five)/i,
    /make (some|a few|5|five) posts/i,
    /create (some|a few|5|five) posts/i,
    /write (some|a few|5|five) posts/i,
    /building (a|an) .{3,50}(app|website|product|service|tool)/i,
    /developing (a|an) .{3,50}(app|website|product|service|tool)/i,
    /creating (a|an) .{3,50}(app|website|product|service|tool)/i,
    /working on (a|an) .{3,50}(app|website|product|service|tool)/i,
    /launching (a|an) .{3,50}(app|website|product|service|tool)/i,
    /promoting (a|an|my) .{3,50}(app|website|product|service|tool|business)/i,
    /post ideas/i,
    /different ideas/i,
    /promotional posts/i
  ];
  
  // Check for questions that might indicate a request for help
  const containsQuestion = userIdea.includes('?');
  
  // Check for specific keywords that strongly indicate a content creation request
  const containsPromotionKeywords = [
    'promote', 'marketing', 'advertise', 'advertisement', 'promotional',
    'launch', 'announce', 'introducing', 'new app', 'new product'
  ].some(keyword => userIdea.toLowerCase().includes(keyword));
  
  // Check if the input is a request rather than a statement to be polished
  const isRequestNotStatement = contentCreationPatterns.some(pattern => pattern.test(userIdea)) || 
                               (containsQuestion && containsPromotionKeywords);
  
  console.log("[API] Content creation detection:", {
    userIdea: userIdea,
    containsQuestion: containsQuestion,
    containsPromotionKeywords: containsPromotionKeywords,
    matchesPatterns: contentCreationPatterns.some(pattern => pattern.test(userIdea)),
    isRequestNotStatement: isRequestNotStatement
  });
  
  const isContentCreationRequest = isRequestNotStatement;
  
  let prompt;
  
  if (isContentCreationRequest) {
    // Content creation prompt - generate new content based on the user's request
    prompt = `I need help creating 5 distinct social media posts based on the following request:
...
1. Create 5 COMPLETELY DIFFERENT posts that fulfill my request
2. Each post should have a unique angle or approach
3. Make the posts sound like they were written by me, matching my style from the 'About me' section
4. Each post must be under 280 characters and ready to publish
5. DO NOT use hashtags in any of the posts
6. DO NOT ask questions in the posts unless specifically requested
7. Make the posts engaging, concise, and directly addressing what I asked for
...
9. Do not add any numbering, labels, or other text before or after each post`;
    ...
    // Original polishing prompt - create slight variations of the user's text
    prompt = `I have a post idea that I need help cleaning up. Please create 5 SLIGHT variations of my post, focusing on minor improvements in grammar, clarity, and conciseness without changing my meaning or style significantly.
