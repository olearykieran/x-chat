  ...
  try {
    ...
    return new Promise((resolve) => {
      chrome.runtime.sendMessage(
        { type: 'FETCH_REAL_TRENDING_TOPICS' },
        (response) => {
          if (chrome.runtime.lastError) {
            console.log('[API] Error fetching trending topics from background:', chrome.runtime.lastError.message);
...
- Writing Style Reference (Examples of their posts): "${profileBio || 'Not provided. In this case, use a general, engaging, and authentic social media style.'}"
...
6. Present each of the 5 posts separated ONLY by the delimiter '###POST_SEPARATOR###'. Do not add any other text, numbering, or formatting before or after the posts or the separator.`;
  ...
  try {
    ...
    let posts = fullResponse.split('###POST_SEPARATOR###').map(post => {
    ...
    if (posts.length < 5 && fullResponse.includes('\n\n')) {
      console.warn('[API] Primary separator "###POST_SEPARATOR###" yielded less than 5 posts. Attempting fallback split by double newline.');
      posts = fullResponse.split('\n\n').map(post => {
        let cleanedPost = post.replace(/^\d+\.\s*/, '');
    ...
    if (posts.length > 5) {
      console.warn(`[API] Expected 5 posts, but got ${posts.length}. Returning the first 5.`);
      return posts.slice(0, 5);
    }
    if (posts.length < 5) {
      console.warn(`[API] Expected 5 posts, but only generated ${posts.length} valid posts after parsing. Raw response: ${fullResponse}`);
