...
export async function generatePolishedPosts(apiKey, userIdea, userProfile) {
  ...
  const prompt = `I have an idea for a post and I need help turning it into 5 polished, ready-to-publish variations. Each variation should capture the essence of my idea but express it in a unique, well-crafted way.
...
1. Create 5 COMPLETELY DIFFERENT variations of my post idea
2. Each version should maintain my core message but express it in a distinct way
3. Each post must be under 280 characters
4. DO NOT use hashtags in any of the posts
5. Match my writing style based on my bio
6. Maintain my authentic voice - these should sound like ME, not generic
7. Present each post separated ONLY by the delimiter '###POST_SEPARATOR###'
8. Do not add any numbering, labels, or other text before or after each post`;
